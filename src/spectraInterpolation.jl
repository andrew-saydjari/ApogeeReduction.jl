...
function returnWeights_inv(obsCoordall::AbstractVector, obsBitMsk::Vector{Int},
        pixindx::AbstractVector, targVal::Float64, cindx::Int;
    ...
    if length(diffwav) == 0 || length(diffpixind) == 0
        return zeros(Int, 2 * kernsize), NaN * ones(2 * kernsize)
    end
    ...
    if linFallBack & (count(mskb) > 1)
        ...
        if !isnothing(lindx) & !isnothing(rindx)
            if (pixOffset[lindx] >= -1) & (pixOffset[rindx] <= 1)
                ...
                return [indvecrr[lindx], indvecrr[rindx]],
                [1 - abs(offlst[lindx]) / totoff, 1 - abs(offlst[rindx]) / totoff]
            end
        end
    end
    return zeros(Int, 2 * kernsize), NaN * ones(2 * kernsize)
...
function generateInterpMatrix_sparse_inv(
        waveobs::AbstractVector, obsBitMsk::Vector{Int}, wavemod::AbstractVector,
        pixindx::AbstractVector; kernsize::Int = 4, linFallBack::Bool = true)
