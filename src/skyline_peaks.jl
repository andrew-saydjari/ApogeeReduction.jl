...
function get_sky_peaks(flux_vec,tele,chip,roughwave_dict,df_sky_lines)
    # try
        ...
        seg_rough_wavs = map(x->rough_linear_wave(mean(x); a=roughwave_dict[tele][chip][1], b=roughwave_dict[tele][chip][2]), segments)
        ...
        bright_lines0 = sort(
            filter(row -> (wav_range[1] <= row.wav*10 <= wav_range[2]) .& (row.number_of_lines .== 2), df_sky_lines),
            :intensity,
            rev=true
        )[1:8,:]        
        ...
        function get_subline_params(lindx)
            ...
            subline_wav_diffs = (parse.(Float64, split(replace(replace(bright_lines[ref_indx,:subline_wav][2:end-1], "  " => ","), " " => ","), ",")[1:2]).-bright_lines[ref_indx,:wav]).*10
            subline_wav_pix_diffs = subline_wav_diffs./(roughwave_dict[tele][chip][2])
            subLine_weight = parse.(Float64, split(replace(bright_lines[ref_indx,:subline_I][2:end-1], " " => ","), ","))
            ...
            function gfit(p; exclude_idx=nothing)
                lam1 = p[2] - p[4]/2
                lam2 = p[2] + p[4]/2
                model = p[1]*subLine_weight[1]*normal_pdf.(x .- lam1, p[3]) .+ p[1]*subLine_weight[2]*normal_pdf.(x .-lam2, p[3]) .+ p[5].*(x.-cpix) .+ p[6]
                residuals = (y .- model).^2
            ...
            outpix = [fitparams[2] - fitparams[5]/2, fitparams[2] + fitparams[5]/2]' * subLine_weight
