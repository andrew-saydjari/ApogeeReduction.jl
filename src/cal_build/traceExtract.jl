    ...
    for r_ind in 1:n_iter
        ...
        if r_ind == 1
            ...
            if !use_first_guess_heights
                ...
                best_model_fluxes_unit_height = gaussian_int_pdf(best_model_fit_inds,curr_guess,return_unit=true)
            ...
            mean_heights = nansum(model_fluxes_unit_height' .* flux_diffs .* fit_ivars, 1)[1,:] ./ ivar_heights
#             @show mean_heights
        ...
        param_offsets .*= 0
        param_offsets[:,2] .= dmu
        model_fluxes_unit_height_above_mu = gaussian_int_pdf(fit_inds,curr_guess .+ param_offsets,return_unit=true)
        param_offsets[:,2] .= -dmu
        model_fluxes_unit_height_below_mu = gaussian_int_pdf(fit_inds,curr_guess .+ param_offsets,return_unit=true)

        param_offsets .*= 0
        param_offsets[:,3] .= dsig
        model_fluxes_unit_height_above_sig = gaussian_int_pdf(fit_inds,curr_guess .+ param_offsets,return_unit=true)
        param_offsets[:,3] .= -dsig
        model_fluxes_unit_height_below_sig = gaussian_int_pdf(fit_inds,curr_guess .+ param_offsets,return_unit=true)
        ...
        for j in 1:curr_n_peaks
            ...
            if !return_cov                
                v_hat[:,j] = (curr_M_T_dot_V_inv * M_vects[:,:,j]) \ (curr_M_T_dot_V_inv * flux_diffs[:,j])
            else
...
function trace_extract(image_data, ivar_image, tele, mjd, chip, expid; image_mask = nothing,mid=1025,n_center_cols=100)
