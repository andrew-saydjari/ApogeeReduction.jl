...
function gaussian_int_pdf(x_vals, params; return_derive = false)
    ...
    cdf = (0.5 * erf.(z_above ./ (2 ^ 0.5))) 
       .- (0.5 * erf.(z_below ./ (2 ^ 0.5))) 

    if return_derive
        dcdf_dmu = (2*π) ^ (-0.5) .* (exp.(-1 .* z_above .^ 2 ./ 2) .- exp.(-1 .* z_below .^ 2 ./ 2)) .* (-1 ./ params[:, 3])
        dcdf_dsig = (2*π) ^ (-0.5) .* (exp.(-1 .* z_above .^ 2 ./ 2) .* (-z_above ./ params[:, 3]) .- exp.(-1 .* z_below .^ 2 ./ 2) .* (-z_below ./ params[:, 3]))
...
function fit_gauss_and_bias(all_rel_fluxes, all_rel_ivars, first_guess_params,
    ...
    for r_ind in 1:n_iter
        ...
        # CDF version
        model_fluxes_unit_height,dmodel_dmu,dmodel_dsig = gaussian_int_pdf(fit_inds, curr_guess, return_deriv=true)
        ...
        flux_diffs .+= best_model_fluxes[
            :, offset_inds .+ (size(best_model_fluxes, 2) .÷ 2) .+ 1]'
        flux_diffs .-= model_fluxes' .+ curr_guess[:, 4]
...
- ``:
"""
function get_firstGuessParams(fluxfpi,ivarfpi,goodfpi,fiber_waveParams,fiber_chipPolyParams;bias_porder=10)
...
function get_initial_fpi_peaks(flux,ivar)
    ...
    flux_thresh = nanzeropercentile(smoothed_fluxes,percent_vec=[70]) 
    ...
    local_max_inds = findall((slopes[1:(end - 1)] .>= 0) .& (slopes[2:end] .<= 0) .&
			     ...
			     (smoothed_fluxes[(begin+1):(end-1)] .>= flux_thresh)) .+ 1
    ...
    good_max_inds = (abs.(relative_fluxes .- 1.0) .< 0.2) .& (local_max_waves .>= 11) .&
                    (local_max_waves .<= n_pixels - 11)
    ...
    new_params = fit_gauss_and_bias(flux, ivar, first_guess_params,
        fit_inds, best_model_fit_inds, offset_inds;
        n_iter = 10, dmu = 0.001, dsig = 0.001, return_cov = false,
        use_first_guess_heights = false, max_center_move = 3,
        min_widths = 0.1, max_widths = 3.0)
    ...
    med_peak_space_spacing = nanmedian(peak_space_spacing ./ (peak_locs[2:end] .- peak_locs[1:(end-1)]))
    ...
    for r_ind in 1:2
        ...
        smoothed_biases .= nansum(
            ((new_params[:, 1] .* keep_biases)[all_smooth_inds])' .* smooth_biases, 1)'
        smoothed_biases ./= nansum(keep_biases[all_smooth_inds]' .* smooth_biases, 1)'
    end
    ...
    new_params,v_hat_cov = fit_gauss_and_bias(flux, ivar, first_guess_params,
        fit_inds, best_model_fit_inds, offset_inds;
        n_iter = 10, dmu = 0.001, dsig = 0.001, return_cov = true,
        use_first_guess_heights = true, max_center_move = 3,
        min_widths = 0.1, max_widths = 3.0)

    return new_params,v_hat_cov
end


function get_and_save_sky_peaks(fname)
    sname = split(fname, "_")
    ...
    get_initial_fpi_peaks(x,flux,ivar)

    function get_peaks_partial(flux_1d,ivar_1d)
        get_initial_fpi_peaks(flux_1d, ivar_1d)
    end
    pout = map(get_partial, eachcol(flux_1d .- med_flux_1d))
    ...
    if isnothing(first_valid_idx)
        println("No valid FPI lines found for ANY fiber in$(fname)")
        return
    end
    sky_line_mat = zeros(
