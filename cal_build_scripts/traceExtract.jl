...
function trace_extract(image_data, ivar_image, tele, mjd, chip, expid; image_mask = nothing)
    ...
    for r_ind in 1:n_iter
        ...
        fit_fluxes .+= best_model_fluxes[
            :, offset_inds .+ (size(best_model_fluxes, 2) .÷ 2)]
    ...
    for r_ind in 1:n_iter
        ...
        new_params[:, 2] .= clamp.(
            clamp.(new_params[:, 2], all_peak_locs .- 3, all_peak_locs .+ 3), 11, 2048 - 11)
    ...
    @showprogress for (ind, x_ind) in enumerate(sorted_x_inds)
        ...
        elseif x_ind > 1025
            first_guess_params = nanmedian(param_outputs[(x_ind - 51):(x_ind - 1), :, :], 1)[
                1, :, :]
        elseif x_ind < 1025
            first_guess_params = nanmedian(param_outputs[(x_ind + 1):(x_ind + 51), :, :], 1)[
                1, :, :]
        end
        ...
        for r_ind in 1:n_iter
            ...
            new_params = curr_guess .+ v_hat
            new_params[:, 1] .= clamp.(
                new_params[:, 1], 0.01, first_guess_params[:, 1] * 10)
            new_params[:, 2] .= clamp.(
                clamp.(new_params[:, 2], first_guess_params[:, 2] .- 3,
                    first_guess_params[:, 2] .+ 3),
                11,
                2048 - 11)
            new_params[:, 3] .= clamp.(new_params[:, 3], 0.5, 2.0)
